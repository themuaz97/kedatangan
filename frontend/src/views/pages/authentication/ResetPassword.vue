  ...
  >
    ...
    >
      ...
      <form class="mt-4 space-y-4 lg:mt-5 md:space-y-5" @submit.prevent="resetPassword">
        ...
        <button
          type="submit"
          class="w-full bg-purple-500 text-white hover:bg-purple-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-purple-800"
        >
          Reset password
        </button>
      </form>
...
const resetPassword = async () => {
  ...
  try {
    await fetch("http://localhost:3005/api/auth/reset-password", {
      method: 'POST',
      headers: {
